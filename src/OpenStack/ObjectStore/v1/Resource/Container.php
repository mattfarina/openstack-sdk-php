...
class Container implements \Countable, \IteratorAggregate
{
     ...
     *
     * @throws \OpenStack\Common\Transport\LengthRequiredException      if the Content-Length could not be determined and
     *                                                                  chunked encoding was not enabled. This should not
     *                                                                  occur for this class, which always automatically
     *                                                                  generates Content-Length headers. However, 
     *                                                                  subclasses could generate this error.
     * @throws \OpenStack\Common\Transport\UnprocessableEntityException if the checksum passed here does not match the
     *                                                                  checksum calculated remotely.
     * @throws \OpenStack\Common\Exception                              when an unexpected (usually network-related) error
     *                                                                  condition arises.
     */
     ...
     *
     * @throws \OpenStack\Common\Transport\FileNotFoundException if the object does not already
    ...
    protected function loadExtraData()
    {
        ...
        if (empty($this->url) || empty($this->token)) {
            throw new \OpenStack\Common\Exception('Remote data cannot be fetched. Tokena and endpoint URL are required.');
        }
